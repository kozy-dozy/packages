...
import { getToastPortalWrap } from './toastPortalConfig'
...
const ToastWrapper = forwardRef((props: ToastWrapperProps, ref: any) => {
    ...
    return (
        <div
            style={{ ...placementTransition.default, ...(block ? { width: '100%' } : {}) }}
            {...rest}
            ref={(thisRef) => {
...
ToastWrapper.getInstance = (props: ToastWrapperProps) => {
    ...
    return new Promise((resolve) => {
        ...
        // App context (Redux store, ThemeProvider, i18n) for this detached root
        // is re-established by whatever the app registered via
        // `configureToastPortal`. Defaults to an identity wrapper.
